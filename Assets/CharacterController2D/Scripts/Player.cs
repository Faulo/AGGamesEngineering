...
namespace AGGE.CharacterController2D {
    public class Player : MonoBehaviour {
        ...
        bool horizontalInputReleased;
        bool skipVerticalDrag;
        Vector3 nextPos;
        float jumpDir;

        float jumpDelayTimer;
        float jumpLandDelayTimer;
        float jumpReleaseDelayTimer;
        float fallTimer;
        float airTimer;
        float jumpCacheTimer;

        //Collision
        float rayOffset = 0.002f;
        float collisionWidthHalf;
        float collisionHeightHalf;
        ...
        void Jump() {
            ...
            if (useJumpUpwards && Mathf.Abs(inputX) < deadZoneX) {
                ...
                var velocityBeforeTakeoff = velocity;
        ...
        bool CollisionCheck(int offsetX, int offsetY, Vector2 dir) {
            Vector2 origin;
            origin.x = previousPos.x + ((collisionWidthHalf - rayOffset) * offsetX);
            origin.y = previousPos.y + ((collisionHeightHalf - rayOffset) * offsetY);

            var hit = Physics2D.Raycast(origin, dir, 3f, defaultMask);
